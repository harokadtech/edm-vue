var myapp = new Vue({
    ...
    methods: {
        doSearch: function(e, startFrom = 0) {
            ...
            const searchHost = 'http://194.146.38.32:8228/document_file/_search';
            var body;
...
function buildQuery(searchTerm, start, numberItems) {
    var body = {'size': numberItems,
                ...
                '_source': ['fileTitle', 'nodePath','name']
               };
    var query = {'bool': {}};
    query.bool.must = {
        'multi_match': {
            'query': searchTerm,
            'fields': ['nodePath', 'fileContent'],
            'fuzziness': 'AUTO'
        }
    };
    body.query = query;
...
function parseResults(jsonResponse) {
    ...
    for (var i = 0; i < jsonResponse.hits.hits.length; i++) {
        ...
        const path = hit._source.nodePath;
        const fname = hit._source.name;
        const meta = hit._source.fileTitle;
        const title  = (typeof meta !== 'undefined' ? hit.name : fname);
